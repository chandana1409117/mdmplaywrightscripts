  ...
  - generic [ref=e19]:
    ...
    - generic [ref=e26]:
      ...
      - link "Back to Login" [ref=e48] [cursor=pointer]:
        - /url: /login
        - img [ref=e49]
        - generic [ref=e51]: Back to Login
      - paragraph [ref=e53]: Need help? Contact support for assistance
```
...
> 12 |     await LoginPage.resetPassword(testdata.resetPassword.newPassword, testdata.resetPassword.confirmPassword);
  ...
  37 | });
  38 | test('should throw error when password and confirm password do not match', async ({ LoginPage, testdata }) => {
  39 |     await LoginPage.navigate(testdata.adminUrl);
  40 |     await LoginPage.forgotPassword(testdata.forgotPassword.email);
  41 |     await LoginPage.resetPassword(testdata.resetPassword.newPassword, 'invalidpassword');
  42 |     await expect(LoginPage.page.getByText('Passwords do not match')).toBeVisible();
  43 | });
  44 | test("show throw error when password is last 5 used passwords", async ({ LoginPage, testdata }) => {
  45 |     await LoginPage.navigate(testdata.adminUrl);
  46 |     await LoginPage.forgotPassword(testdata.forgotPassword.email);
  47 |     await LoginPage.resetPassword(testdata.resetPassword.last5Passwords[0], testdata.resetPassword.last5Passwords[0]);
  48 |     await expect(LoginPage.page.getByText('You cannot reuse your last 5 passwords.')).toBeVisible();
  49 | });
  50 | });
```